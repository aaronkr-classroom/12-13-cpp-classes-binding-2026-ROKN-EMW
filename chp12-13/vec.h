...
public:
	...
	*/
	...
	//인터페이스 = 미완성....
	void push_back(const T& val)//나중에...
	{
...
private:
	...
	allocator<T> alloc; //메모리 할당을 위해 관리하는 객체
...
{// int i = 0; lhs = left-hand side (i) = 변수, rhs = right-hand side (0) = 값 
	//자가 할당 여부 확인
...
template <class T> void Vec<T>::create(size_type n, const T& val)
{
	...
	limit = avail = data + n;
	uninitailized_fill(data, limit, val);
}
template <class T> 
...
// 사용 가능한 공간은 다 채우면 벡터크기를 2배로 증가하는 함수
template <class T>
void Vec<T>::grow()
{
